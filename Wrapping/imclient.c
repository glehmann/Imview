...
#ifdef HAVE_SYSV_IPC
static ipctype use_shm_default = SHM_SYSV; 
#else
...
#endif
#ifdef HAVE_POSIX_IPC
    if (conn_array[conn_id].use_shm == SHM_POSIX) { /* using POSIX IPC */
...
{
  char cmd[ANSWER_MAX_SIZE];
  char *res;

  if ((conn_id1!=conn_id2) && conn_array[conn_id1].opened && conn_array[conn_id2].opened) 
    {
    sprintf(cmd, "link %s %d\r\n", conn_array[conn_id2].hostname, conn_array[conn_id2].port);
    res = imviewsendcommand(cmd, conn_id1);
    return 0; /* if we get here all is well */
    } 
  else 
    {
    return 1;
